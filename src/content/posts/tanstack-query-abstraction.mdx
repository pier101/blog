export const metadata = {
  title: "우리가 TanStack Query를 잘못 추상화해온 방법",
...
# 우리가 TanStack Query를 잘못 추상화해온 방법

> **TkDodo의 "Creating Query Abstractions"를 읽고 정리한 글입니다.**  
...
개발자로서 작업을 할 때 조금이라도 공통성이 보이면 간결하게 리팩토링 하는 것에 대한 강박이 조금씩 들곤 하는데요.

TanStack Query를 사용하다 보면 여러 컴포넌트에서 같은 `queryKey`와 `queryFn`을 반복해서 쓰게 되고, 이 중복을 줄이기 위해 **커스텀 훅**을 써보자! 라고 생각이 듭니다.

이 글에서는 그 커스텀 훅 방식이 어떤 함정을 품고 있는지, 그리고 TanStack Query v5가 제시하는 더 나은 추상화 방법인 `queryOptions`가 왜 올바른 선택인지를 단계적으로 살펴보아요.
